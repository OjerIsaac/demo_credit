...
/**
 * @description user withdraw fund
 * @param req Request object
 * @param res Response object
 * @returns ErrorResponse | SuccessResponse
 */
export const withdrawUserFund = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const { amount } = req.body;
        ...
        if (Number(userBalance[0].wallet) < Number(amount)) {
            return errorResponse(res, httpErrors.AccountError, "Insufficient Balance");
        }
        ...
        return successResponse(res, "Withdrawal successful", { newBalance });
    } catch (error) {
      console.log(error);
      return errorResponse(res, httpErrors.ServerError, "Something went wrong");
    }
